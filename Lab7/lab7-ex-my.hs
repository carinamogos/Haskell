...
randoms :: Int -> [Word8]
randoms = create.initial
initial = mkStdGen
create gen = (fromIntegral ((fst.next) gen)) : (create ((snd.next)gen))
...
tableToFunc tabela caract 
	| caract == (fst.head) tabela = (snd.head) tabela
	|otherwise = tableToFunc (tail tabela) caract
...
xorCrypt s = zipWith xor (map charToWord s)
...
xorCrypt2 s k
	| length s == length k =  xorCrypt s k
	| otherwise =  xorCrypt s $ (take.length) s  $ myCycle k
...
language alf = [""] ++ concatMap (\ x -> (map (\y -> y : x)  alf)) (language alf)
