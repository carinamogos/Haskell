...
randoms :: Int -> [Word8]
randoms seed = randFromGen $ mkStdGen seed

randFromGen x = fromIntegral (mod (fst rez) 256) : (randFromGen (snd rez))
  where rez = next x
...
--xorCrypt  = zipWith (xor.charToWord)  -- merge si fara
xorCrypt s = zipWith (xor.charToWord) s
...
wrap s k = take (length s) (myCycle k)

xorCrypt2 :: String -> [Word8] -> [Word8]
xorCrypt2 s k = xorCrypt s (wrap s k)
...
--rationament -> mereu generez sirurile cu 1 mai mare

language :: [Char] -> [String]
language alf = [""]++(generateFromAlf alf [""])

generateFromChr char content = map (\x -> x ++ [char] ) content

generateFromAlf alf content = rez ++ (generateFromAlf alf rez)
                          where rez = foldl (\acc x -> acc++(generateFromChr x content)) [] alf
